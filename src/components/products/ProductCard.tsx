...
import { Card } from '@/components/ui/card'; // Card component itself is used
import type { Product } from '@/lib/types';
import { WishlistButton } from '@/components/features/WishlistButton';
import { StarRating } from '@/components/shared/StarRating';
import { cn } from '@/lib/utils';
import { buttonVariants, Button } from '@/components/ui/button'; // Import Button explicitly
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="block group h-full" passHref>
      <Card className="flex flex-col rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 ease-in-out bg-card border border-border hover:border-primary/30 h-full cursor-pointer">
        {/* Image Section */}
        ...
        <div className="p-3 flex flex-col flex-grow">
          ...
          <Button
            variant="default"
            size="sm"
            className="w-full mt-2 text-xs py-2 px-3" // Uses default primary bg from buttonVariants
            onClick={(e) => {
              e.preventDefault(); 
